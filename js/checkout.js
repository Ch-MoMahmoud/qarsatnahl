(function(){
  ...
  const SHIPPING_FLAT = 50; // flat shipping value
  // Integrations
  ...
  const calcTotals = ()=>{
    const subtotal = Object.entries(cartState).reduce((sum, [id,qty])=>{
      const p = getProduct(id); if(!p) return sum; return sum + (Number(p.price)||0) * (Number(qty)||0);
    }, 0);
    const shipping = subtotal>0 ? SHIPPING_FLAT : 0;
    const grand = subtotal + shipping;
    SUBTOTAL_EL && (SUBTOTAL_EL.textContent = priceFmt(subtotal));
    SHIPPING_EL && (SHIPPING_EL.textContent = priceFmt(shipping));
  ...
  const buildLineItems = (state)=>{
    ...
    Object.entries(state||{}).forEach(([id, qtyRaw])=>{
      ...
      const origUnitPrice = Number(p.compareAtPrice || p.price || 0);
      items.push({
        id: p.id,
        title: p.title,
        unitPrice,
    ...
    ids.forEach((id)=>{
      ...
      li.innerHTML = `
        ...
        <div class="meta">
          ...
          <div class="pr">${priceFmt(p.price)}</div>
        </div>
        <div class="qty">
  ...
  const validate = ()=>{
    if(!FORM) return true;
    const required = ['fullName','address','city','phone','email'];
    let ok = true;
    required.forEach(id=>{
